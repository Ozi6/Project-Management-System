...
const GeneralSettings = ({ setShowAdvanced, projectId }) => {
    ...
    return(
        <div className="space-y-4 sm:space-y-6 max-w-2xl mx-auto bg-[var(--gray-card3)] p-4 sm:p-6 rounded-xl shadow-xl">
            <div className="flex items-center space-x-2">
                <FaCog className="text-[var(--features-title-color)] text-2xl sm:text-3xl" />
                <h2 className="text-2xl sm:text-3xl font-semibold text-[var(--features-text-color)]">{t("set.gen")}</h2>
            </div>

            {!isProjectOwner && (
                <div className="p-2 sm:p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-xs sm:text-sm text-yellow-700 flex items-center">
                    <UserCheck className="w-4 h-4 sm:w-5 sm:h-5 mr-2 text-yellow-500" />
                    {t("set.per")}
                </div>
            )}

            {saveSuccess && (
                <div className="p-2 sm:p-3 bg-green-50 border border-green-200 rounded-lg text-xs sm:text-sm text-green-700 flex items-center">
                    <FaSave className="w-4 h-4 sm:w-5 sm:h-5 mr-2 text-green-500" />
                    Project settings saved successfully!
                </div>
            )}

            <div className="bg-[var(--bg-color)] rounded-xl shadow-lg p-4 sm:p-8 space-y-4 sm:space-y-6">
                <div className="space-y-1 sm:space-y-2">
                    <label className="block text-xs sm:text-sm font-medium text-[var(--text-color3)]">{t("set.name")}</label>
                    <input
                        type="text"
                        value={projectName}
                        onChange={(e) => isProjectOwner && setProjectName(e.target.value)}
                        className={`block w-full rounded-xl border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-xs sm:text-sm p-2 sm:p-3 ${
                            isProjectOwner
                ...
                <div className="space-y-1 sm:space-y-2">
                    <label className="block text-xs sm:text-sm font-medium text-[var(--text-color3)]">{t("set.d")}</label>
                    <textarea
                ...
                <div className="space-y-1 sm:space-y-2">
                    <label className="block text-xs sm:text-sm font-medium text-[var(--text-color3)]">Due Date</label>
                    <input
                        type="date"
                        value={dueDate}
                ...
                <div className="space-y-1 sm:space-y-2">
                    <label className="block text-xs sm:text-sm font-medium text-[var(--features-text-color)]">{t("set.update")} (800x400)</label>
                    <input
                        type="file"
                        accept="image/*"
                        ...
                        } file:mr-4 file:py-1.5 sm:file:py-2 file:px-3 sm:file:px-4 file:rounded-full file:border-0 file:text-xs sm:file:text-sm file:font-semibold`}
                        disabled={!isProjectOwner}
                    />
                    <div className="mt-3 sm:mt-4 relative w-full h-40 sm:h-64 bg-[var(--gray-card3)]/30 rounded-xl shadow-md">
                        <img
                            src={backgroundImage}
                ...
                <div className="flex flex-wrap gap-2 sm:gap-0 sm:flex-nowrap sm:justify-between">
                    <button
                        onClick={() => setShowAdvanced(true)}
                        className="flex-1 sm:flex-initial bg-[var(--features-icon-color)]/50 hover:bg-[var(--hover-color)] !text-white font-semibold py-2 sm:py-3 px-4 sm:px-6 rounded-xl text-xs sm:text-sm transition duration-200 ease-in-out"
                    >
                        {t("set.advanced")}
                    </button>
                    ...
                    {isProjectOwner && (
                        <button
                            ...
                            disabled={saving}
                            className="flex-1 sm:flex-initial flex items-center justify-center gap-1 sm:gap-2 bg-green-500 hover:bg-green-600 !text-white font-semibold py-2 sm:py-3 px-4 sm:px-6 rounded-xl text-xs sm:text-sm transition duration-200 ease-in-out"
                        >
                    ...
                    {isProjectOwner ? (
                        <button
                            ...
                            className={`flex-1 sm:flex-initial bg-red-500 !text-white font-semibold py-2 sm:py-3 px-4 sm:px-6 rounded-xl text-xs sm:text-sm transition duration-200 ease-in-out ${
                        ...
                        <button
                            onClick={handleDeleteClick}
                            className={`flex-1 sm:flex-initial bg-red-500 !text-white font-semibold py-2 sm:py-3 px-4 sm:px-6 rounded-xl text-xs sm:text-sm transition duration-200 ease-in-out hover:bg-red-600`}
                        >
                            {t("set.leave")}
                        </button>
                    )}
                        ...
                        >
                            ...
                            >
                                <div className="flex items-center mb-3 sm:mb-4">
                                    ...
                                    <h3 className="text-base sm:text-lg font-bold ml-2 sm:ml-3 text-gray-800">
                                ...
                                <div className="flex justify-end space-x-2 sm:space-x-3">
                                    ...
                                    <button
                                        ...
                                        className="px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors flex items-center"
                                    >
                                        {isProjectOwner ? (
                                            <Trash2 className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-1.5" />
                                        ) : (
                                            <DoorOpen className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-1.5" />
                                        )}
                                        {isProjectOwner ? t("procard.del") : t("procard.leave")}
                                    </button>
                                </div>
