...
const Sidebar = ({ activeTab, setActiveTab }) => {
    ...
    const navItems = [
        { id: 'dashboard', icon: Layout, label: 'Dashboard', path: '/dashboard', color: 'bg-slate-100 text-slate-600' },
        { id: 'team', icon: Users, label: 'Teams', path: '/team', color: 'bg-green-100 text-green-600' },
        { id: 'calendar', icon: Calendar, label: 'Calendar', path: '/calendar', color: 'bg-amber-100 text-amber-600' },
        { id: 'settings', icon: Settings, label: 'Settings', path: '/project/settings', color: 'bg-slate-100 text-slate-600' },
        ...
        <div className="flex">
            ...
            <div className={`flex-1 transition-all duration-300 ${isOpen ? 'ml-64' : 'ml-16'} p-6`}>
                <button
                    ...
                    className={`fixed z-20 top-[50vh] ${isOpen ? 'left-[256px]' : 'left-[72px]'} w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center cursor-pointer shadow-md hover:bg-blue-700 transition-all duration-200`}
