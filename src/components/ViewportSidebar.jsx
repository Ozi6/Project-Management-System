...
const Sidebar = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <div className="flex">
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 h-screen bg-white transition-all duration-300 shadow-md z-10 overflow-hidden 
                ${isOpen ? "w-64" : "w-16"}`}
      >
        <div className="sidebar-content flex flex-col h-full">
          <Link
            to="/"
            className="p-4 flex items-center space-x-2 no-underline hover:bg-gray-50 transition-colors duration-200"
          >
            <Layout className="h-8 w-8 text-gray-600" />
            {isOpen && (
              <h1 className="text-2xl font-bold text-blue-600">PlanWise</h1>
            )}
          </Link>
