        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {Object.entries(kanbanBoard).map(([status, tasks], columnIndex) => (
              <div key={status} className="bg-gray-50 p-6 rounded-xl">
                <h3 className="font-semibold mb-4 capitalize">{status.replace(/([A-Z])/g, ' $1').trim()}</h3>
                <div className="space-y-3">
                  {tasks.map((task, i) => (
                    <motion.div 
                      key={i} 
                      className="bg-white p-3 rounded shadow-sm border-l-4 border-blue-500 hover:shadow-md transition-shadow cursor-pointer"
      ...
      <div className="mb-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <motion.div 
              key={index}
              className="p-6 bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-1 border-t-4 border-blue-500"
              initial={{ opacity: 0, y: 20 }}
            ...
            <div className="flex flex-col md:flex-row gap-8 items-center">
              <div className="md:w-1/2">
                ...
                <ul className="space-y-2">
                  ...
                  <li className="flex items-start">
                    <CheckSquare className="h-5 w-5 text-green-500 mr-2 mt-0.5" />
                    <span>Bulk task creation for efficiency</span>
                  </li>
                  <li className="flex items-start">
                    <CheckSquare className="h-5 w-5 text-green-500 mr-2 mt-0.5" />
                    <span>Task templates for common activities</span>
                  </li>
                </ul>
              </div>
              <div className="md:w-1/2 bg-white p-4 rounded-lg shadow-md">
                <div className="bg-gray-50 p-3 rounded mb-3">
                  <div className="flex justify-between mb-2">
                    ...
                    <div className="text-blue-600 text-sm">Save as template</div>
                  </div>
                  ...
                  <div className="bg-white border border-gray-200 rounded p-2 mb-2">
                    ...
                    <div className="flex">
                      <div className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center mr-1">
                        <span className="text-xs text-blue-600">JD</span>
                      </div>
                      <span>John Doe</span>
                    </div>
                  </div>
            ...
            <div className="flex flex-col md:flex-row-reverse gap-8 items-center">
              <div className="md:w-1/2">
                ...
                <ul className="space-y-2">
                  <li className="flex items-start">
                    <CheckSquare className="h-5 w-5 text-green-500 mr-2 mt-0.5" />
                    <span>Customizable progress indicators</span>
                  </li>
                  <li className="flex items-start">
                    <CheckSquare className="h-5 w-5 text-green-500 mr-2 mt-0.5" />
